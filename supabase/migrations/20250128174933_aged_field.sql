/*
  # Fix UNION columns in yearly highlights function

  1. Changes
    - Ensures all CTEs have the same columns for UNION
    - Adds engagement_score to all CTEs for consistency
    - Maintains existing functionality

  2. Technical Details
    - Adds engagement_score calculation to all CTEs
    - Uses COALESCE to handle NULL values
    - Maintains existing highlight types and logic
*/
...
BEGIN
  ...
  top_memories AS (
    SELECT 
      ...
      COALESCE((metadata->>'rating')::integer, 0) as engagement_score
    FROM memories
    WHERE 
  ...
  active_periods AS (
    SELECT DISTINCT ON (month)
      ...
      mc.memory_count as engagement_score
    FROM memories m
